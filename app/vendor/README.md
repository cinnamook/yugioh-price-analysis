...
`app/build_app.py` inlines this file into the generated page, so the service worker
...
shasum -a 256 app/vendor/supabase.umd.js   # record the new hash in this file
python3 app/build_app.py
```

Check the bundle contains no `</script` sequence after updating (`grep -ci '</script'`
should print 0); `app/build_app.py` escapes it defensively, but a hit is worth knowing about.

...
@media(min-width:901px){.bpmenu{left:50%;right:auto;transform:translateX(-50%);width:min(900px,calc(100% - 44px))}}
...
function setPhase(p){if(!board)return;board.phase=p;renderSim();}
...
function pmenuHTML(){ if(!pmenu||!board)return '';
  ...
  return '<div class=bpmenu><span class=btsel>'+esc(PILE_LABEL[k]||k)+' &middot; '+n+'</span>'
    ...
    +'<span class=btsep></span><button onclick="pmClose()">Close</button></div>';}
